...
: QDialog(parent), m_GroupCount(count)
{
    ...
    connect(m_OKButton, SIGNAL(clicked()), this, SLOT(m_acccept()));
...
void MultiInputDialog::m_acccept()
{
    this->close();

    Ok_Button_Down = true ;
    //保留m_dlgmsg中指令名称，生成新的msg列表m_dlgmsg
    QStringList msg;

    msg = MultiInputDialog::GetAllTexts();

    msg.prepend(this->windowTitle());
...
QStringList MultiInputDialog::receiveData(QStringList list)
{
    return list;
